...
- Format 함수 - CString
  ...
  - 사용법
    - GDI 객체를 스택에 생성한다.
    - 생성된 GDI 객체를 디바이스 컨텍스트에 선택하고 이전에 선택되어 있던 같은 종류의 GDI 객체의 주소를 저장해둔다. (`CDC::SelectObject()`사용)
    - GDI 함수를 사용하여 출력을 한다.
    - 이전의 GDI 객체를 디바이스 선택함으로써 기존에 선택된 GDI 객체를 선택 해제한다.(`CDC::SelectObject()` 사용)
